...
func (room room) isRealRoom() bool {
	return room.correctChecksum == room.checksum
}
...
func getRooms() []room {
	...
	for index := 0; index < len(lines); index++ {
		rooms[index] = parseRoom(lines[index])
	}
	return rooms
}

func parseRoom(line string) room {
	roomSpec := regexp.MustCompile(`([a-z-]+)(\d+)\[([a-z]+)\]`)
	matches := roomSpec.FindStringSubmatch(line)
	if matches == nil {
		panic("no matches")
	}
...
func calculateCheckSum(roomName string) string {
	normalizedName := strings.Replace(roomName, "-", "", -1)
	counters := make(map[rune]int)
	for _, codePoint := range normalizedName {
		counters[codePoint]++
	}
	runeCounts := make([]runeCount, len(counters))
	index := 0
	for key, value := range counters {
		runeCounts[index] = runeCount{codePoint: key, count: value}
		index++
	}
	sort.Sort(runeCountSorter(runeCounts))
	firstRunes := make([]rune, 5)
	for i, v := range runeCounts[0:5] {
...
type runeCountSorter []runeCount

func (s runeCountSorter) Len() int {
	return len(s)
}

func (s runeCountSorter) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

func (s runeCountSorter) Less(i, j int) bool {
	one := s[i]
	other := s[j]
	if one.count == other.count {
		return one.codePoint < other.codePoint
	}
